function calculateBoxes() {
  ...
  if (boxes > 0) {
    if (boxes === 1) {
      if (remainder > 0) {
        result.textContent = `Es ${boxes} caja con ${unitsPerBox} unidades y una caja con ${remainder} unidades.`;
      } else {
        result.textContent = `Es ${boxes} caja con ${unitsPerBox} unidades.`;
      }
    } else {
      if (remainder > 0) {
        result.textContent = `Son ${boxes} cajas con ${unitsPerBox} unidades y una caja con ${remainder} unidades.`;
      } else {
